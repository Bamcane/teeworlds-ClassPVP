...
int CServer::LoadMap(const char *pMapName)
{
...

	{
		...
		char aClientMapDir[IO_MAX_PATH_LENGTH];
		str_format(aClientMapDir, sizeof(aClientMapDir), "clientmaps");
		
		if(!Storage()->CreateFolder(aClientMapDir, IStorage::TYPE_SAVE))
		{
			dbg_msg("ClassPVP", "Can't create the directory '%s'", aClientMapDir);
		}
		
		str_format(aClientMapDir, sizeof(aClientMapDir), "clientmaps/classpvp");
		...
		if(!MapConverter.CreateMap(pMapName))
			return 0;
			
		m_aCurrentMapCrc[MAP_TYPE_SIX] = dfServerMap.Crc();

		m_aCurrentMapSha256[MAP_TYPE_SIX] = dfServerMap.Sha256();
		dfServerMap.Close();
		...
		char aBufMsg[128];
		str_format(aBufMsg, sizeof(aBufMsg), "map crc is %08x", m_aCurrentMapCrc[MAP_TYPE_SIX]);
		Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "server", aBufMsg);
		str_format(aBufMsg, sizeof(aBufMsg), "map sha256 is %s", aSha256);
		Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "server", aBufMsg);
	...
	{
		char aBufMsg[128];
		str_format(aBuf, sizeof(aBuf), "clientmaps/classpvp/%s7.map", pMapName);
		IOHANDLE File = Storage()->OpenFile(aBuf, IOFLAG_READ, IStorage::TYPE_ALL);
