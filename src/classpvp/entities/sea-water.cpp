...
#include <classpvp/effects/dehydration.h>
#include "sea-water.h"
...
void CSeaWater::Tick()
{
    ...
    for(int i = 0;i < MAX_CLIENTS;i ++)
    {
        ...
        if(!pChr)
            continue;
        if(pChr->GetCID() == m_Owner || GameServer()->m_pController->IsFriendlyFire(m_Owner, pChr->GetCID()))
			continue;
        if(distance(pChr->m_Pos, CurPos) < 96.0f)
		{
            pChr->GetCore()->m_Vel += normalize(CurPos - PrevPos);
			pChr->AddEffect(new CDehydration(pChr, Server()->Tick(), 200, 8, m_Owner, m_Weapon));
		}
	}
