...
void CCharacter::HandleWeaponSwitch()
{
	...
	if(!m_pPlayer->GetClass())
	{
		if(Next < 128) // make sure we only try sane stuff
		{
			...
			{
				m_pPlayer->m_MapMenuItem = (m_pPlayer->m_MapMenuItem+1)%(int)Server()->Classes()->m_Classes.size();
				Next--;
			}
		}
		...
		if(Prev < 128) // make sure we only try sane stuff
		{
			...
			{
				m_pPlayer->m_MapMenuItem = (m_pPlayer->m_MapMenuItem-1)<0?(int)Server()->Classes()->m_Classes.size()-1:m_pPlayer->m_MapMenuItem-1;
				Prev--;
			}
		}
		return;
...
bool CCharacter::GiveWeapon(int Weapon, int Ammo)
{
	...
	{
		GameServer()->m_pController->GiveWeapon(GetClass(), Weapon);
		m_aWeapons[Weapon].m_Got = true;
		m_aWeapons[Weapon].m_Ammo = Ammo;
		return true;
	}

	if(m_aWeapons[Weapon].m_Ammo < g_pData->m_Weapons.m_aId[Weapon].m_Maxammo)
	{
...
void CCharacter::HandleMenu()
{
	...
	GameServer()->m_pController->OnPlayerSelectClass(m_pPlayer, m_LatestInput.m_Fire&1);
}
