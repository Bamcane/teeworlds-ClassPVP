...
: IGameController(pGameServer)
{
	...
	GameServer()->Console()->ExecuteFile("config/ctf.cfg");
}
		...
		if(F->m_pCarryingCharacter)
		{
			...
			if(m_apFlags[fi^1] && m_apFlags[fi^1]->m_AtStand)
			{
				if(distance(F->m_Pos, m_apFlags[fi^1]->m_Pos) < CFlag::ms_PhysSize + CCharacter::ms_PhysSize)
				{
					...
					int CaptureTime = (Server()->Tick() - F->m_GrabTick)/Server()->TickSpeed();
					if(CaptureTime <= 60)
					{
						GameServer()->SendChatTarget_Localization(-1, _("The {str:Team} flag was captured by '{str:Player}'! ({sec:Time})"), 
							"Team", fi ? _C("on blue flag captured", "blue") : _C("on red flag captured", "red"),
							"Player", Server()->ClientName(F->m_pCarryingCharacter->GetPlayer()->GetCID()),
							"Time", &CaptureTime, NULL);
					}
					else
					{
						GameServer()->SendChatTarget_Localization(-1, _("The {str:Team} flag was captured by '{str:Player}'!"), 
							"Team", fi ? _C("on blue flag captured", "blue") : _C("on red flag captured", "red"),
							"Player", Server()->ClientName(F->m_pCarryingCharacter->GetPlayer()->GetCID()), 
							NULL);
					}
